...
u8 mem2base[MEM2_MAX_SIZE] __attribute__((aligned(32)))__attribute__(());//(section(".ARM.__at_0X68000000")));					//外部SRAM内存池
u8 mem3base[MEM3_MAX_SIZE] __attribute__((aligned(32)))__attribute__(());//(section(".ARM.__at_0X10000000")));					//内部CCM内存池
//内存管理表
u16 mem1mapbase[MEM1_ALLOC_TABLE_SIZE];													//内部SRAM内存池MAP
u16 mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__(());//(section(".ARM.__at_0X68000020")));	//外部SRAM内存池MAP
u16 mem3mapbase[MEM3_ALLOC_TABLE_SIZE] __attribute__(());//(section(".ARM.__at_0X10000020")));	//内部CCM内存池MAP
//内存管理参数	   
...
//c :要设置的值
//num:需要设置的内存大小(字节为单位)
__attribute__((noinline)) void my_mem_set(void *s,u8 c,u32 num)  
{  
    u8 *xs = s;  
    while(num--)*xs++=c;  
}	   
...
//内存管理初始化  
//memx:所属内存块
__attribute__((noinline)) void my_mem_init(u8 memx)  
{  
...
//memx:所属内存块
//返回值:使用率(0~100)
__attribute__((noinline)) u8 my_mem_perused(u8 memx)  
{  
    u32 used=0;  
    u32 i;  
...
inline u32 my_mem_malloc(u8 memx,u32 size)  
{  
...
//offset:内存地址偏移
//返回值:0,释放成功;1,释放失败;  
inline u8 my_mem_free(u8 memx,u32 offset)  
{  
    int i;  
...
//memx:所属内存块
//ptr:内存首地址 
__attribute__((noinline)) void myfree(u8 memx,void *paddr)  
{  
	u32 offset;   
...
//size:内存大小(字节)
//返回值:分配到的内存首地址.
__attribute__((noinline)) void *mymalloc(u8 memx,u32 size)  
{  
    u32 offset;   
...
//size:要分配的内存大小(字节)
//返回值:新分配到的内存首地址.
__attribute__((noinline)) void *myrealloc(u8 memx,void *paddr,u32 size)  
{  
    u32 offset;    
    offset=my_mem_malloc(memx,size);   	
    if(offset==0XFFFFFFFF)return NULL;     
